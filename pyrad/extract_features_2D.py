...
def feature_extractor(inputPath, maskPath, outPath):
  ...
  mainDir = os.path.dirname(os.path.dirname(inputPath))
  params = os.path.join(mainDir, 'Params.yaml') #look for parameters file for pyradiomics
  ...
  try:
    imagefiles, maskfiles = load_image_and_masks(inputPath, maskPath)
...
def main(inputPath, maskPath, outPath):
    feature_extractor(inputPath, maskPath, outPath)