...
def load_image_and_masks(inputfile, maskfile):
    _filenames = []
    _masks = []
    supported_extensions = ('*.tif', '*.tiff')
    if platform.system() == 'Windows':
        #inputfile = os.path.normpath(inputfile)
        path_to_image = os.path.normpath(inputfile)
        path_to_mask = os.path.normpath(maskfile)
...
def feature_extractor(inputfile, maskfile, outputfile, nrofthreads):

    imagefiles, maskfiles = load_image_and_masks(inputfile, maskfile)
    ...
    for ii in range(len(imagefiles)):
        ...
        outputfilename = os.path.normpath(os.path.join(outputfile, "{}.csv".format(output_filename)))
        ...
        #print('extraction till here is good.')
        ...
        for i in result_table:
            for k in i:
                if counter == 0:
                    with open(outputfilename, 'a') as opfl:
                        writer = csv.DictWriter(opfl, fieldnames=list(k.keys()), lineterminator='\n')
                        writer.writeheader()
                        writer.writerow(k)
                        opfl.close()
                else:
...
def main(inputfile, maskfile, outputfile, nrofthreads):
    # print("main")
    feature_extractor(inputfile, maskfile, outputfile, nrofthreads)
