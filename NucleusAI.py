...
from PyQt5.QtWidgets import QDesktopWidget, QMessageBox, QDialog, QApplication, QFileDialog, QMainWindow, QVBoxLayout, \
    QListWidgetItem, QSlider, QStyleFactory, QSizePolicy, qApp, QTextEdit
from PyQt5.QtWebEngineWidgets import QWebEngineView
from PyQt5.QtGui import QTransform, QIcon, QIntValidator, QFont
from PyQt5.QtCore import QUrl, QObject, pyqtSignal, QSize, QEvent, Qt
...
class Resizer(QtCore.QObject):
    ...
    def eventFilter(self, obj, event):
        if self.widget is obj and event.type() == QEvent.Resize:  # changed adg
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.qlabel2.setFont(QFont('Arial', 10))
        self.pushjumpframe = QtWidgets.QPushButton('Go')
        self.pushjumpframe.setFont(QFont('Arial', 10))
        ...
        self.pushextract.setEnabled(False)  # added adg
    ...
    def warningbox(self, message):
        ...
        self.msg.setText(message + str(self.supported_extensions))
    ...
    def training_thread(self):
        ...
        try:
            if self.inputfileRaw != '' and self.inputfilemask != '' and self.opfname != '':
                self.pushtrain.setEnabled(False)
    ...
    def ipbrowsefiles(self):
        ...
        if not self.inputfileRaw:
            return

        if not self.inputfilemask:
            return
        ...
        if platform.system() == 'Windows':
            self.inputfileRaw = os.path.normpath(self.inputfileRaw)
            self.inputfilemask = os.path.normpath(self.inputfilemask)

        self.path_to_image = self.inputfileRaw

        try:
            self.path_to_masks = self.inputfilemask
        ...
        if not self._filenames:
            self.warningbox("Please select a valid input directory with files containing the extensions: ")
        elif not self._masknames:
            self.warningbox("Please select a valid mask directory with files containing the extensions: ")

        self.MainDir = os.path.dirname(self.path_to_image)
        print('Root directory: ', self.MainDir)
        print('Directory for raw images: ', self.path_to_image)

        if self.path_to_masks != None:
            print('Directory for labelled images: ', self.path_to_masks)
        ...
        if self._filenames:
            ...
            self.inputdirtext.setText(self.MainDir)
    ...
    def opbrowsefiles(self):
        self.opfname = QFileDialog.getExistingDirectory(self, 'Choose the directory to save the model', '')
    ...
    def prediction_page_ip(self):
        ...
        self.pred_ipfilename = QFileDialog.getExistingDirectory(self, 'Select folder containing raw images', '')
        ...
        self.pred_path_to_image = self.pred_ipfilename

        for ext in self.supported_extensions:
            self.pred_filenames.extend(glob.glob(self.pred_path_to_image + self.slash + ext))

        if not self.pred_filenames:
            self.warningbox("Please select a valid input directory with files containing the extensions: ")

        self.MainDir = os.path.dirname(self.pred_ipfilename)

        print('Root directory: ', self.MainDir)
        print('Directory for raw images: ', self.pred_path_to_image)

        if self.pred_filenames:
            self.image_loaded = True
            self.inputdirtext_2.setText(self.MainDir)
            self._current_index = 0
    ...
    def prediction_page_op(self):
        self.pred_opfilename = QFileDialog.getExistingDirectory(self,
                                                                'Choose the directory to save the segmented images', '')
    ...
    def prediction_page_modeldir(self):
        self.pred_modeldir = QFileDialog.getExistingDirectory(self, 'Open model directory', '')
    ...
    def validation_page_ip(self):
        ...
        if not self.val_ipfilenameRaw:
            return
        if not self.val_ipfilenamemask:
            return
        ...
        if platform.system() == "Windows":
            self.val_ipfilenameRaw = os.path.normpath(self.val_ipfilenameRaw)
            self.val_ipfilenamemask = os.path.normpath(self.val_ipfilenamemask)

        self.val_path_to_image = self.val_ipfilenameRaw
        try:
            self.val_path_to_masks = self.val_ipfilenamemask
        except:
        ...
        if not self.val_filenames:
            self.warningbox("Please select a valid input directory with files containing the extensions: ")
        elif not self._masknames:
            self.warningbox("Please select a valid mask directory with files containing the extensions: ")

        self.MainDir = os.path.dirname(self.val_ipfilenameRaw)

        print('Root directory: ', self.MainDir)
        print('Directory for raw images: ', self.val_path_to_image)

        if self.val_path_to_masks != None:
            print('Directory for labelled images: ', self.val_path_to_masks)
            assert len(self.val_filenames) == len(self._masknames), "Number of images and masks does not match"
            self.mask_loaded = True

        if self.val_filenames:
            self.image_loaded = True
            self.inputdirtext_5.setText(self.MainDir)
            self._current_index = 0
            self.display_images(self.val_filenames[self._current_index])
    ...
    def validation_page_op(self):
        self.val_opfilename = QFileDialog.getExistingDirectory(self, 'Choose the root directory to save the results',
                                                               '')
    ...
    def validation_page_modeldir(self):
        self.val_modeldir = QFileDialog.getExistingDirectory(self, 'Open model directory', '')
    ...
    def extraction_page_ip(self):
        ...
        if not self.extract_ipfilenameRaw:
            return
        if not self.extract_ipfilenameMask:
            return

        if platform.system() == "Windows":
            self.extract_ipfilenameRaw = os.path.normpath(self.extract_ipfilenameRaw)
            self.extract_ipfilenameMask = os.path.normpath(self.extract_ipfilenameMask)
        #

        self.MainDir = os.path.dirname(self.extract_ipfilenameRaw)

        if self.extract_ipfilenameRaw:
            self.inputdirtext_4.setText(self.MainDir)
            # print('input dir is set.')
        else:
            return

        print('Root directory: ' + self.MainDir)
        print('Directory for raw images: ' + self.extract_ipfilenameRaw)
        print('Directory for segmented images: ' + self.extract_ipfilenameMask)

        self.extract_path_to_image = self.extract_ipfilenameRaw
        try:
            self.extract_path_to_masks = self.extract_ipfilenameMask
        except:
    ...
    def extraction_page_op(self):
        self.extract_opfilename = QFileDialog.getExistingDirectory(self,
                                                                   'Choose the root directory to save the results', '')
        if platform.system() == "Windows":
    ...
    def display_images(self, image_path, FrameIndex=0):  # NEW
        ...
        if show_stack:
            self.imagelabel.setText(self.filenm)
        ...
        if len(self._masknames) > 0 and (self.maskoncheck.isChecked() or self.outlinesoncheck.isChecked()):
            ...
            self.mask_image = overlay.draw_layer(image_path=image_path, masks_path=self.maskfile,
                                                 currentZ=self.currentZ, masksOn=masksOn, outlinesOn=outlinesOn)
    ...
    def json_save(self):
        ...
        self.config['patchSizeD'] = self.inputs['Patch Size Depth'].text()
    ...
    def save_training_parameters(self):
        ...
        self.param['PatchD'] = self.inputs['Patch Size Depth']
    ...
    def training_param(self):
        ...
        if not self.save_para:
            self.inputs['Model Name'] = QtWidgets.QLineEdit(text='model_ddmmyy')
            self.inputs['Model Name'].setToolTip('Enter name of the model.')
            self.inputs['Validation Fraction'] = QtWidgets.QLineEdit(text='.1')
            self.inputs['Validation Fraction'].setToolTip(
                'The fraction of available data that is used for validation, default')
            ...
            self.inputs['Patch Size Depth'] = QtWidgets.QLineEdit()
            self.inputs['Patch Size Depth'].setToolTip(
                "Size (Depth) of the image patches used to train the network, only with 3D data. Must be divisible by 4. Should be less than the minimum number of slices in Z.")
            self.inputs['Patch Size Depth'].setPlaceholderText("Only for 3D")
            ...
            self.inputs['Epochs'].setToolTip('Number of training epochs')
            ...
            self.inputs['N_rays (Recommended to use a power of 2.)'].setToolTip(
                'Number of radial directions for the star-convex polygon. Recommended to use a power of 2')
            ...
            self.inputs['Patch Size Depth'] = self.param['PatchD']
    ...
    def validation_param(self):
        ...
        if not self.val_save_para:
            ...
            self.val_inputs['2D'].setToolTip('Select to validate with 2D model')
        ...
        # self.w.setFixedSize(600, 350)
        self.val_w.setWindowTitle('Set Parameters for validation')
        self.val_w.show()
    ...
    def save_extraction_parameters(self):
        ...
        self.pushextract.setEnabled(True)  ##added adg
    ...
    def training(self, thread_count, log):
        ...
        try:
            self.config = dict()
            self.config['MainDir'] = self.MainDir
            self.config['inputDir'] = self.inputfileRaw
            self.config['maskDir'] = self.inputfilemask
            ...
            raise MyException("An error occurred. Please check if all the inputs are given.")

        # if self.runlocal.isChecked():
        run_local(self.config, destination='training')
        log.finished.emit("thread %d" % thread_count)
        self.pushpredict.setEnabled(True)  # added adg
    ...
    def predict(self, thread_count, log):
        self.pushpredict.setEnabled(False)
        self.train_parameters.setEnabled(False)
        self.pushtrain.setEnabled(False)
        self.pushjson.setEnabled(False)
        self.pushtboard.setEnabled(False)
        ...
        log.finished.emit("thread %d" % thread_count)
        self.train_parameters.setEnabled(True)
    ...
    def predict_with_trained_model(self, thread_count, log):
        ...
        self.config = dict()
        self.config['inputDir'] = self.pred_ipfilename
        ...
        self.pushpredict_2.setEnabled(False)
        self.pred_parameters.setEnabled(False)
        run_local(self.config, destination='prediction_from_trained_model')

        log.finished.emit("thread %d" % thread_count)
        # self.pushpredict_2.setEnabled(True)
        self.pred_parameters.setEnabled(True)
    ...
    def validate_trained_model(self, thread_count, log):
        self.pushvalidate_5.setEnabled(False)
        self.val_parameters.setEnabled(False)
        self.config = dict()
        self.config['MainDir'] = self.inputdirtext_5.text()
        self.config['inputDir'] = self.val_ipfilenameRaw
        self.config['maskDir'] = self.val_ipfilenamemask
        ...
        avg_prec = np.round(np.mean(self.scores_dict['precision']), 2)
        ...
        self.validation_textbox.setAlignment(Qt.AlignLeft)
        self.validation_textbox.append("Accuracies for individual validations")
        for i, j in zip(self.scores_dict['image_name'], self.scores_dict['accuracy']):
            self.validation_textbox.append("\n" + str(i) + ':' + str(round(j, 2)))

        self.validation_textbox.append('\n' + "Overall IOU: " + str(avg_iou) +
                                       '\n' + "Overall Precision: " + str(avg_prec) +
                                       '\n' + "Overall Recall: " + str(avg_rec) +
                                       '\n' + "Overall Accuracy: " + str(avg_acc) +
                                       '\n' + "Overall Dice: " + str(avg_dice) +
                                       '\n' + "Overall AUC: " + str(avg_auc))

        log.finished.emit("thread %d" % thread_count)
        self.pushval_csv.setEnabled(True)
        self.val_parameters.setEnabled(True)
    ...
    def feature_extraction(self, thread_count, log):
        ...
        self.extract_param_push.setEnabled(False)

        self.config = dict()
        self.config['MainDir'] = self.inputdirtext_4.text()
        self.config['inputDir'] = self.extract_ipfilenameRaw
        self.config['maskDir'] = self.extract_ipfilenameMask
        ...
        print('Feature Extraction parameters ::: ', self.config)

        run_local(self.config, destination='feature_extraction')

        log.finished.emit("thread %d" % thread_count)
        self.extract_param_push.setEnabled(True)
...
if __name__ == "__main__":
    ...
    while currentExitCode == MainWindow.EXIT_CODE_REBOOT:
        ...
        app.setWindowIcon(QIcon(
            os.path.join(user_path(), 'Documents/AI/StarDist_GUI/Imaging_tools-main_2022-09-11/static/stardist.icns')))
